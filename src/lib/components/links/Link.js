...
class Link extends Component {
  ...
  calcSCoordinates = () => {
    const cpt1 = {
      x: this.props.start.x + SSHAPE_SIDE_WIDTH,
      y: this.props.start.y,
    };
  ...
  getPath = () => {
    ...
    if (this.props.start.x > this.props.end.x) {
      ...
      return `M${this.props.start.x} ${this.props.start.y}  ${
        coordinates.cpt1.x
      } ${coordinates.cpt1.y} ${coordinates.cpt2.x} ${coordinates.cpt2.y} ${
        coordinates.cpt3.x
      } ${coordinates.cpt3.y} ${coordinates.cpt4.x} ${coordinates.cpt4.y} ${
        this.props.end.x
      } ${this.props.end.y}`;
    }
