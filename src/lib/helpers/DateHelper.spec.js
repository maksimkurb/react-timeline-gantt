...
describe('Test DateToPixel Fuctionality', () => {
  ...
  test('When Today and now position  0', () => {
    const result = DateHelper.dateToPixel(DateHelper.getToday(), 0, DAY_WIDTH);
    expect(result).toBe(0);
  });
  test('When Today and now position  100 ', () => {
    const result = DateHelper.dateToPixel(
      DateHelper.getToday(),
      100,
      DAY_WIDTH,
    );
  ...
  test('When tomorrow and now position 0', () => {
    const tomorrow = DateHelper.getToday();
    tomorrow.setDate(tomorrow.getDate() + 1);
    const result = DateHelper.dateToPixel(tomorrow, 0, DAY_WIDTH);
    expect(result).toBe(DAY_WIDTH);
  });
  test('When tomorrow and now position 100', () => {
    const tomorrow = DateHelper.getToday();
    tomorrow.setDate(tomorrow.getDate() + 1);
    const result = DateHelper.dateToPixel(tomorrow, 100, DAY_WIDTH);
    expect(result).toBe(124);
  });
  test('When yesterday and now position 0', () => {
    const yesterday = DateHelper.getToday();
    yesterday.setDate(yesterday.getDate() - 1);
    const result = DateHelper.dateToPixel(yesterday, 0, DAY_WIDTH);
    expect(result).toBe(-DAY_WIDTH);
  });
  test('When yesterday and now position 100', () => {
    const yesterday = DateHelper.getToday();
    yesterday.setDate(yesterday.getDate() - 1);
    const result = DateHelper.dateToPixel(yesterday, 100, DAY_WIDTH);
...
describe('Test date difference', () => {
  test('Test with in the same month', () => {
    ...
    const start = new Date(2018, 9, 8);
    const end = new Date(2018, 9, 18);
    const result = DateHelper.monthDiff(start, end);
    expect(result).toBe(0);
  });
  ...
  test('Test different month same year', () => {
    // (start,end,now,dayWidth)
    const start = new Date(2018, 9, 8);
    const end = new Date(2019, 11, 18);
    const result = DateHelper.monthDiff(start, end);
    expect(result).toBe(14);
  });
  test('Test different  start > end', () => {
    // (start,end,now,dayWidth)
    const start = new Date(2019, 9, 8);
    const end = new Date(2018, 11, 18);
    const result = DateHelper.monthDiff(start, end);
